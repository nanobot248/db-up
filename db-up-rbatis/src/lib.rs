...
impl MigrationExecutor for RbatisMigrationDriver {
    async fn begin_transaction(&self) -> db_up::Result<()> {
    ...
    async fn execute_changelog_file(&self, changelog_file: db_up::ChangelogFile) -> db_up::Result<()> {
    ...
    async fn commit_transaction(&self) -> db_up::Result<()> {
    ...
    async fn rollback_transaction(&self) -> db_up::Result<()> {
